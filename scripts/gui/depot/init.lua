...
-- Traindepot.Gui: Fenster des Depots. Teil: click-handlers.
...
function Traindepot.Gui:onInit()
  if not storage.TD_data.Gui then
    storage.TD_data.Gui = self:initGlobalData()
  end
end
...
function Traindepot.Gui:setOpenedEntity(playerIndex, openedEntity)
  storage.TD_data.Gui["openedEntity"][playerIndex] = openedEntity
end
...
function Traindepot.Gui:getDepotGuiLayout()
  return storage.TD_data.Gui["prototypeData"]["trainDepotGui"]
end

function Traindepot.Gui:getTabElementPath(guiElementName)
  return storage.TD_data.Gui["prototypeData"]["tabButtonPath"][guiElementName]
end

function Traindepot.Gui:getUpdateElementPath(guiElementName)
  return storage.TD_data.Gui["prototypeData"]["updateElementPath"][guiElementName]
end
...
function Traindepot.Gui:getOpenedEntity(playerIndex)
  return storage.TD_data.Gui["openedEntity"][playerIndex]
end
...
function Traindepot.Gui:updateGuiInfo(playerIndex)
  ...
  local openedEntity                                                                       = self:getOpenedEntity(
  playerIndex)
  local depotName                                                                          = openedEntity and
  openedEntity.valid and openedEntity.backer_name or ""
  local depotForceName                                                                     = openedEntity and
  openedEntity.valid and openedEntity.force.name or ""
  local depotSurfaceIndex                                                                  = openedEntity and
  ...
  -- statistics ----------------------------------------------------------------
  local depotStationCount                                                                                         = Traindepot
...
require("scripts.gui.depot.click-handlers")
