...
-- Alle Ereignisse der Mod: Lebenszyklus (on_init, on_load, on_configuration_changed),
-- Bauen/Abbauen mit Filtern, GUI. Die Logik liegt in den Bereichen unter scripts/.

local onConfigChanges = require("scripts.core.config-changed.init")
...
local function onRemovedEntity(event)
  ...
  if removedEntity and removedEntity.valid then
    ...
    Trainassembly:onRemoveEntity(removedEntity, event.buffer)
